...
if (Object.hasOwn(manifest, 'mcpServers') || manifest.interface?.defaultPrompt?.length > 3 || !manifest.description?.includes('street-furniture')) throw new Error('Некорректные discovery-метаданные плагина')
if (sidecar.schema_version !== 1 || sidecar.agent?.entry_skill !== 'hobbyka-catalog-agent') throw new Error('Некорректный sidecar')
if (!agentMetadata.includes('allow_implicit_invocation: true') || agentMetadata.includes('type: "mcp"') || !agentMetadata.includes('$hobbyka-catalog-agent')) throw new Error('Некорректные метаданные навыка')
...
if (!skill.startsWith('---\nname: hobbyka-catalog-agent\n') || !/обычной задаче по комплектации благоустройства/i.test(skill) || !/среде без запуска локального CLI сообщить/i.test(skill)) throw new Error('Некорректный скилл')
