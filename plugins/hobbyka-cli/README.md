...
Официальный открытый плагин Hobbyka для ИИ-агентов с локальным терминалом, версия `0.8.0`. Агент использует CLI для поиска товаров, локального сопоставления по фотографии, запроса проектных материалов, проверки серверных цен и подготовки коммерческих предложений.
...
Менеджер после входа может подготовить «Трикапэ» из существующего КП: `offer tricape options --id ID` показывает отправителей и ID фото каждого товара, затем `offer tricape create --id ID --stdin` принимает JSON с `markup_percent`, `sender` и необязательным массивом `photo_ids`. Результат содержит `html_url`; PDF сохраняют через печать открытого документа. Требуются `admin_all_offers`, `offers.admin.read` и `offers.pricing.write`.
