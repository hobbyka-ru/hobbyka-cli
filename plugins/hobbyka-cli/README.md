...
Официальный открытый плагин Hobbyka для ИИ-агентов. Агент работает через компактный CLI, а не напрямую с API, MCP, глобальным поиском или HTML-каталогом сайта. Рабочая публичная среда: [new.hobbyka.ru/ai/](https://new.hobbyka.ru/ai/).
...
- `HOBBYKA_BASE_URL` — переопределение адреса среды; по умолчанию используется `https://new.hobbyka.ru`;
